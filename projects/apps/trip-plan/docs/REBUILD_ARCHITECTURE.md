...
코드/문서/frontend/backend는 모두 `projects/apps/trip-plan/` 안에서 관리하고, 실제 여행 자료는 앱 코드와 분리해 관리한다.

현재 Node 구현은 `projects/apps/trip-plan/legacy-node/`로 이동했다. 초기 migration 기간에는 같은 앱 디렉터리 안에서 보관하되, 기본 개발 경로는 Kotlin backend와 React frontend다.
...
- legacy prototype 입력 데이터 파일은 로컬 전용 자료로만 취급
