...
기존 Node prototype은 `legacy-node/`에 보관했습니다. 실제 여행 데이터는 generic app seed와 분리해 관리합니다.

## Rebuild Plan

범용 AI 여행 플래너로 재구현하기 위한 문서:

- `docs/REBUILD_ARCHITECTURE.md`: Kotlin/Spring + React 전체 아키텍처
- `docs/REBUILD_DB_SCHEMA.md`: SQLite/Flyway 목표 스키마
- `docs/REBUILD_SEED_DATA.md`: 별도 SQL 기반 초기 데이터 방침
- `docs/REBUILD_API.md`: REST/SSE API 계약
- `docs/REBUILD_AI_OPERATIONS.md`: AI 편집 operation 계약
- `docs/REBUILD_AI_PROVIDERS.md`: 로컬/외부 AI provider 추상화 계획
- `docs/REBUILD_CODEX_APPSERVER.md`: Codex app-server 연동 계획
- `docs/REBUILD_FRONTEND.md`: React/Tailwind 프론트엔드 계획
- `docs/REBUILD_MIGRATION_PLAN.md`: 단계별 마이그레이션 계획
...
- `docs/`: 재구현 문서
- `legacy-node/`: 현재 Node prototype 보관

목표 스택:

- Kotlin 2.3.20
- Spring Boot 4.0.5
- JDK 21 (`/Library/Java/JavaVirtualMachines/temurin-21.jdk/Contents/Home`)
- SQLite + Flyway
...
서버는 시작 시 `legacy-node/db/schema.sql`을 적용하고 `legacy-node/.data/trip-planner.sqlite`를 생성합니다. 기존 prototype의 `legacy-node/data/` 입력 파일은 로컬 전용 자료이며 git 관리 대상에서 제외합니다.
...
AI 편집 구조와 operation 스키마는 `docs/AI_EDITING.md`에 정리되어 있습니다.
...
Spring Boot backend는 Gradle 모노레포의 `backend` project입니다.

기본 SQLite 파일은 legacy Node DB와 분리된 `.data/trip-planner-backend.sqlite`입니다.
...
React frontend는 `frontend` project입니다. 개발 서버는 backend 8081로 `/api`, `/actuator` 요청을 proxy합니다.
